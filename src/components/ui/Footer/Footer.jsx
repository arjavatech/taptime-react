...
const Footer = () => {
  ...
  const contactInfo = [
    { href: "https://maps.google.com/?q=2135+204th+PL+NE,+Sammamish,+WA+98074", src: "/images/location-pin.png", alt: "Location", text: "Arjava Technologies,\n2135 204th PL NE,\nSammamish,\nWA - 98074.", className: "w-5 mt-1", mb: "mb-3" },
    { href: "tel:+14258181900", src: "/images/phone-call.png", alt: "Phone", text: "(541) 371-2950", className: "w-4 h-4", mb: "mb-2" },
    { href: "https://mail.google.com/mail/?view=cm&fs=1&to=contact@tap-time.com", src: "/images/paper-plane.png", alt: "Email", text: "contact@tap-time.com", className: "w-4 h-4", mb: "" }
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col md:flex-row flex-wrap justify-evenly items-start gap-12 text-center md:text-left">
        ...
        <div className="address text-sm sm:text-base w-full md:w-auto text-center md:text-left">
          ...
          {contactInfo.map((info, i) => (
            <a key={i} href={info.href} target={i === 0 ? "_blank" : undefined} rel={i === 0 ? "noopener noreferrer" : undefined} className="hover:underline text-white">
              <div className={`flex flex-col sm:flex-row items-center justify-center md:justify-start ${info.mb} gap-2`}>
                <img src={info.src} alt={info.alt} className={info.className} />
                {info.text.split('\n').map((line, j) => (
                  <React.Fragment key={j}>{line}{j < info.text.split('\n').length - 1 && <br />}</React.Fragment>
                ))}
              </div>
            </a>
          ))}
